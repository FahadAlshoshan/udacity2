...
import com.udacity.boogle.maps.domain.AddressRepository;
import com.udacity.boogle.maps.domain.Coordinates;
...
public class MapsService {
  ...
  public MapsService(AddressRepository repository) {
    this.repository = repository;
  }

  public Address get(Coordinates primaryKey) {
    Address addressFromDB = repository.findById(primaryKey).orElse(null);
    if (addressFromDB != null) {
      return addressFromDB;
    }
    Address newAddress = generateAddress(primaryKey);
  ...
  private Address generateAddress(Coordinates PrimaryKey) {
    Random generator = new Random();
    int randomIndex = generator.nextInt(ADDRESSES.size());

    String address = ADDRESSES.get(randomIndex);
    ...
    return new Address(PrimaryKey, streetAndNumber, city, state, zip);

  }

  private final List<String> ADDRESSES = List.of("777 Brockton Avenue, Abington MA 2351",
